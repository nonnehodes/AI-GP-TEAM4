...
class HUWebshop(object):
    ...
    def recommendations(self, count, productid='0000'):
        ...
        if resp.status_code == 200:
            ...
            if len(recs) < 4:
                print('Fail. going FBT filter')
                recs = FrontEnd_recs.shopping_cart_rec(productid)
            if len(recs) < 4:
                print('Fail. going random')
    ...
    def recommendations_shoppingcart(self, count, productid='0000'):
        ...
        if resp.status_code == 200 and productid != '0000':
            recs = FrontEnd_recs.shopping_cart_rec(productid)
            if len(recs) < 4:
                print('Fail. going content filter')
                recs = FrontEnd_recs.product_page_rec(productid)
            if len(recs) < 4:
                print('Fail. going random')
                recs = eval(resp.content.decode())
    ...
    def productpage(self, cat1=None, cat2=None, cat3=None, cat4=None, page=1):
        ...
        return self.renderpackettemplate('products.html', {'products': prodlist, \
            ...
            'r_products':self.recommendations_cat_page(4, nononescats[0]), \
    ...
    def shoppingcart(self):
        ...
        if len(i) > 0:
            random.shuffle(i)
            prodid = i[0].get('id')
            print('id cart: ' + str(i[0].get('id')))
        return self.renderpackettemplate('shoppingcart.html',{'itemsincart':i,\
            'r_products':self.recommendations_shoppingcart(4, prodid), \
